...
import nodePath__default, { resolve, isAbsolute, relative, dirname, sep } from 'path';
...
function exportTranslationKeys(keys, locale, config, cache) {
  ...
  for (const [filePath, keysForFilepath] of Object.entries(keysPerFilepath)) {
    ...
    const directoryPath = nodePath__default.dirname(filePath);
...
function plugin (api) {
  ...
  return {
    ...
    visitor: {
      Program(path, state) {
        const toPosixPath = p => p.split(nodePath.sep).join(nodePath.posix.sep);
